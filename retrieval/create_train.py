...
parser.add_argument("--qrels_file", type=str, default="../data/medical/qrels.train.tsv", help="qrels file")
parser.add_argument("--query_file", type=str, default="../data/medical/train.query.txt", help="query file")
parser.add_argument('--collection_file', type=str, default="../data/medical/result.tsv", help="collections file")
parser.add_argument("--save_to", type=str, default="../data/medical/train/train.json", help="processd train json file")
parser.add_argument("--tokenizer_name", type=str, default="bert-base-chinese", help="pretrained model tokenizer")
args = parser.parse_args()

#将训练集的query.txt映射为字典：qmap[qid]=qry id为问题id，qry为问题描述
...
#语料库映射为字典，cmap[did]=content did为文档段落id，content为文档内容
...
with open(args.save_to, 'w') as f:
    with Pool() as p:
        all_tokenized = p.imap_unordered(
            ...
            chunksize=500,    #分块 500
        )
        for x in all_tokenized:
            if x is None:
                continue
            f.write(x + "\n")
#将训练集的qrels.train.tsv分词字符化
# 格式：
